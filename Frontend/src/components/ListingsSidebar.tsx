...
const ListingsSidebar: React.FC<{ activePage: string }> = ({ activePage }) => {
  const showDashboard = isAdminUser(localStorage.getItem('phone'), localStorage.getItem('accountType'), localStorage.getItem('email'));
  const links = [
    ...(showDashboard ? [{ page: 'dashboard', to: '/dashboard', label: 'Dashboard', icon: LayoutDashboard }] : []),
